...
def provjera(mbs):
    ...
    if json_data != 'null':
        return {
            ...
            'maticni_broj_tvrtke': json_object['mbs'],
            'pravni_postupak': json_object['postupci'][0]['vrsta']['znacenje'],
            'temeljni_kapital_tvrtke': json_object['temeljni_kapitali'][0]['iznos'],
            'adresa_sjedista_tvrtke': {'ulica': json_object['sjedista'][0]['ulica'],
                                       'kucni_broj': json_object['sjedista'][0]['kucni_broj'],
                                       'naziv_naselja': json_object['sjedista'][0]['naziv_naselja']}
        }
    else:
        pass
